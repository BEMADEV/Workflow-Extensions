...
namespace com.bemaservices.WorkflowExtensions.Workflow.Action
{
    ...
    /* BEMA.FN1.Start */
    [DefinedValueField( Rock.SystemGuid.DefinedType.COMMUNICATION_SMS_FROM, "From", "The number to originate message from (configured under Admin Tools > Communications > SMS Phone Numbers).", false, false, "", "", 0 )]
    [WorkflowAttribute ( "From Attribute", "Workflow attribute that contains the SMS Phone Number to use for sending. Note that if the From field is entered, it will be used and this attribute will be ignored.", false, "", "", 0, null,
    ...
    public class SendSms : ActionComponent
    {
        ...
        public override bool Execute( RockContext rockContext, WorkflowAction action, Object entity, out List<string> errorMessages )
        {
            ...
            {
                string fromAttr = GetAttributeValue ( action, "FromAttribute" );
                var attrGuid = fromAttr.AsGuid ();
                if ( !attrGuid.IsEmpty () )
                {
                    var attribute = AttributeCache.Get ( attrGuid, rockContext );
                    if ( attribute != null )
                    {
                        string fromAttributeValue = action.GetWorkflowAttributeValue ( attrGuid );
                        if ( !string.IsNullOrWhiteSpace ( fromAttributeValue ) )
                        {
                            switch ( attribute.FieldType.Class )
                            {
                                case "Rock.Field.Types.DefinedValueFieldType":
                                    {
                                        fromGuid = fromAttributeValue.AsGuidOrNull ();
                                        var fromValue = DefinedValueCache.Get ( fromGuid.Value, rockContext );
                                        if ( fromValue != null )
                                        {
                                            fromId = fromValue.Id;
            ...
            if (fromId == null)
            {
                action.AddLogEntry ( "Warning: From SMS Phone Number was supplied so nothing was sent.", true );
                return true;
            }
            /* BEMA.FN1.End */
