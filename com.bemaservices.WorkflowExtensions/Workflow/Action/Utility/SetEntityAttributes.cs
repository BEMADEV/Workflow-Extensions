        ...
        public override bool Execute( RockContext rockContext, WorkflowAction action, Object entity, out List<string> errorMessages )
        {
            ...
            if ( attributeMatrix != null )
            {
                foreach ( AttributeMatrixItem attributeMatrixItem in attributeMatrix.AttributeMatrixItems )
                {
                    ...
                    if ( entityWithAttributes.Attributes.TryGetValue( attributeKey, out attribute ) )
                    {
                        // Handle encrypted text fields as well.
                        if ( attribute.FieldType.Field is Rock.Field.Types.EncryptedTextFieldType )
                        {
                            attributeValue = Rock.Security.Encryption.EncryptString( attributeValue );
                        }
                    }
